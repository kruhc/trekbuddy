...
public class AndroidInputMethod extends InputMethod {
	...
	public void pointerScaled(int x, int y) {
		if (DeviceFactory.getDevice().hasPointerMotionEvents()) {
			MIDletBridge.getMIDletAccess().getDisplayAccess().pointerScaled(x, y);
		}
	}
...
*/
	...
	private int getKeyCode(KeyEvent keyEvent) {
		...
		default: 
			...
			if (resultKeyCode == 0) {
				if (deviceKeyCode == KeyEvent.KEYCODE_BACK) {
					resultKeyCode = KeyEvent.KEYCODE_BACK;
				} else {
