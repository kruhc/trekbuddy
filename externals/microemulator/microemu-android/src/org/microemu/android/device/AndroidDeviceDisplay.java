 ...
 *  limitations.
 *
 *  @version $Id: AndroidDeviceDisplay.java 2341 2010-03-26 11:30:06Z barteo@gmail.com $
 */
...
//		this.opts.inDither = true;
		...
		this.opts.inTempStorage = new byte[16 * 1024]; // from Android source code (Froyo-2.2), should be reused for decoding -> less GC
	...
	public Image createImage(InputStream is) throws IOException {
		return new AndroidImmutableImage(BitmapFactory.decodeStream(is, null, opts));
	}
	...
	public Image createImage(byte[] imageData, int imageOffset, int imageLength) {
		return new AndroidImmutableImage(BitmapFactory.decodeByteArray(imageData, imageOffset, imageLength, opts));
	}
...
//        }
        ((AndroidCanvasUI) DisplayUtils.getDisplayableUI(gameCanvas)).flushGraphics(x, y, width, height);
    }
	...
	public int getColor(int colorSpecifier) {
		...
		switch (colorSpecifier) {
			...
			case Display.COLOR_HIGHLIGHTED_FOREGROUND:
				return (Integer) context.getDisplayColors().get(new Integer(colorSpecifier));
