...
public final class Font 
{
	...
	public static Font getFont(int face, int style, int size, int sizeOverride)
	{
		if (sizeOverride == 0)
			throw new IllegalArgumentException("sizeOverride == 0");
		if (size != 0)
			throw new IllegalArgumentException("size != 0");
		final Font result = new Font(face, style, size);
	...
	public int getSize()
	{
		return size; // returns MIDP size only
	}
	...
	public int hashCode() {
		return style + size + face + (sizeOverride << 16);
	}	
