 ...
 *   Andres Navarro
 *   
 *  @version $Id: Display.java 2381 2010-06-05 10:29:29Z barteo@gmail.com $
 */
...
//			}
		...
		public void pointerPressed(final int x, final int y) {
			if (current != null) {
if (current instanceof GameCanvas) current.pointerPressed(x, y); else 
		...
		public void pointerReleased(final int x, final int y) {
			if (current != null) {
if (current instanceof GameCanvas) current.pointerReleased(x, y); else 
				eventDispatcher.put(eventDispatcher.new PointerEvent(new Runnable() {
		...
		public void pointerDragged(final int x, final int y) {
			if (current != null) {
if (current instanceof GameCanvas) current.pointerDragged(x, y); else 
				eventDispatcher.put(eventDispatcher.new PointerEvent(new Runnable() {
		...
		public void pointerScaled(final int x, final int y) {
			if (current != null) {
if (current instanceof GameCanvas) current.pointerScaled(x, y); else 
				eventDispatcher.put(eventDispatcher.new PointerEvent(new Runnable() {
					
					public void run() {
						current.pointerScaled(x, y);
						
					}
					
				}, EventDispatcher.PointerEvent.POINTER_SCALED, x, y));
			}
		}
...
//		timer.scheduleAtFixedRate(new RunnableWrapper(new GaugePaintTask()), 0, Ticker.PAINT_TIMEOUT);
	...
	public int getColor(int colorSpecifier) {
		return DeviceFactory.getDevice().getDeviceDisplay().getColor(colorSpecifier);
	}
	...
	public void setCurrent(final Displayable nextDisplayable) {
		...
		if (nextDisplayable == current) { /* restore seamless after Alert */
			return;
		}
		...
		if (nextDisplayable != null) {
			...
			eventDispatcher.put(new ShowNotifyEvent(eventDispatcher, new Runnable() {
				...
				public void run() {
					...
					if (current != null && !(nextDisplayable instanceof Alert)) { /* restore will be seamless after Alert */
						eventDispatcher.put(new HideNotifyEvent(eventDispatcher, new Runnable() {
					...
					if (nextDisplayable instanceof Alert) {
						...
						*/
						// current = nextDisplayable; /* restore will be seamless when this Alert goes away */
