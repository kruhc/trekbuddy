 ...
 *  limitations.
 *
 *  @version $Id: AndroidDisplayGraphics.java 2454 2010-12-18 11:01:01Z barteo@gmail.com $
 */
...
public class AndroidDisplayGraphics extends javax.microedition.lcdui.Graphics {
    ...
    public Paint filterPaint = new Paint();
	...
	public void reset(Canvas canvas) {
	    ...
	    final Rect tmp = this.canvas.getClipBounds();
...
//		setFont(Font.getDefaultFont());
	...
	public synchronized void drawChars(char[] data, int offset, int length, int x, int y, int anchor) {
        ...
        if ((anchor & javax.microedition.lcdui.Graphics.HCENTER) != 0) {
            newx -= androidFont.paint.measureText(data, offset, length) / 2;
        } else if ((anchor & javax.microedition.lcdui.Graphics.RIGHT) != 0) {
            newx -= androidFont.paint.measureText(data, offset, length);
        }
        ...
        if (delegate != null) {
	        ...
	        delegate.drawSubstringDelegate(str, 0, str.length(), newx, newy, anchor);
	    } else {    
	        canvas.drawText(data, offset, length, newx, newy, androidFont.paint);
	    }
	}

	public synchronized void clear(int color) {
		canvas.drawRGB((color & 0x00ff0000) >> 16, (color & 0x0000ff00) >> 8, (color & 0x000000ff));
	}		
	...
	public synchronized void drawImage(Image img, int x, int y, int anchor) {
        if (delegate != null) {
            ...
            final Bitmap bmp = ((ImageAccessor) img).getBitmap();
            if (bmp != null && !bmp.isRecycled()) { // when not true??
                targetRect.left = newx;
                targetRect.top = newy;
                targetRect.right = newx + bmp.getWidth();
                targetRect.bottom = newy + bmp.getHeight();
                canvas.drawBitmap(bmp, null, targetRect, null); // bitmap is already filtered
/* does not respect scaled images
                canvas.drawBitmap(bmp, newx, newy, null);
*/
            }
        }
	}

	public synchronized void drawImage(Image img, int x, int y, int anchor, int rotation) {
        if (delegate != null) {
            throw new UnsupportedOperationException("delegate.drawImage(img, x, y, anchor, rotation)");
        } else if (rotation == 0) {
            ...
            if (bmp != null && !bmp.isRecycled()) { // when not true??
                tmpMatrix.reset();
                tmpMatrix.setRotate(rotation, bmp.getWidth() / 2, bmp.getHeight() / 2);
                tmpMatrix.postTranslate(newx, newy);
                canvas.drawBitmap(bmp, tmpMatrix, filterPaint);
/* problematic, see catch clause
                targetRect.left = newx;
                targetRect.top = newy;
                targetRect.right = newx + bmp.getWidth();
                targetRect.bottom = newy + bmp.getHeight();
                canvas.save();
                canvas.rotate(rotation, newx + bmp.getWidth() / 2, newy + bmp.getHeight() / 2);
                canvas.drawBitmap(bmp, null, targetRect, filterPaint);
                try {
                  canvas.restore();
                } catch (IllegalStateException e) {
                  // ignore on Samsung, see http://stackoverflow.com/questions/23893813/canvas-restore-causing-underflow-exception-in-very-rare-cases
                }
*/
            }
        }
	...
	public synchronized void drawPath(Object o) {
        if (delegate != null) {
            delegate.drawPath(o);
        } else {
    			canvas.drawPath((Path)o, strokePaint);
        }
	}
	...
	public synchronized void drawRegion(Image src, int x_src, int y_src, int width,
        ...
        final Bitmap img = ((ImageAccessor) src).getBitmap();
        Bitmap tmpImg;
        ...
        case Sprite.TRANS_NONE: {
        	tmpImg = Bitmap.createBitmap(img, x_src, y_src, width, height);
            break;
        }
        case Sprite.TRANS_ROT90: {
            tmpMatrix.preRotate(90);
        	tmpImg = Bitmap.createBitmap(img, x_src, y_src, width, height, tmpMatrix, true);
            dW = height;
            dH = width;
            break;
        }
        ...
        if (delegate != null) {
            ...
            delegate.drawRegionDelegate(tmpImg, tmpRect, tmpRectSecond);
        } else {
            canvas.drawBitmap(tmpImg, x_dst, y_dst, null);
        }
        tmpImg.recycle();
	}

	public synchronized void drawRGB(int[] rgbData, int offset, int scanlength, int x,
	...
	public synchronized void fillTriangle(int x1, int y1, int x2, int y2, int x3, int y3) {
	    if (delegate != null) {
